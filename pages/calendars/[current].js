...
export const getStaticProps = async ({ params: { current } }) => {
  const { calendar, text, border, color, name, share, tag } =
    calendars[current ?? "python"];
  ...
  return { props: { events: JSON.stringify(events), name, share, tag } };
};

export const getStaticPaths = async () => {
  const searches = Object.keys(calendars).map((key) => key);

  const paths = searches.map((search) => {
    return { params: { current: search } };
  });
